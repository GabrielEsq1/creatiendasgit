...
const formatPrice = (value: string | number, storeCurrency: string = 'COP') => {
    if (typeof value === 'undefined' || value === null) return '0';
    const sanitized = typeof value === 'string'
        ? value.replace(/\./g, '').replace(/,/g, '')
        : value;
    let num = Number(sanitized || 0);
    if (storeCurrency === 'USD') num = num * 4000;
...
export default function StorePreview({ data, products, viewMode = 'desktop', readOnly = false }: StorePreviewProps) {
    ...
    useEffect(() => {
        ...
        if (selectedProduct) url.searchParams.set('p', String(selectedProduct.id));
    ...
    const categoryData = Array.isArray(products)
        ? Array.from(
            products.reduce((map, p) => {
                ...
                if (!map.has(key)) map.set(key, { display: category, image: p.image || null, originalKey: key });
                return map;
    ...
    const containerClass = readOnly
        ? 'store-live min-h-screen flex flex-col'
        : `store-preview-container flex flex-col ${viewMode === 'mobile' ? 'device-mobile' : 'device-desktop'}`;
    ...
    const HighlightMatch = ({ text, keywords }: { text: string; keywords: string[] }) => {
        ...
        return (
            <span>
                {parts.map((part, i) =>
                    regex.test(part)
                        ? <mark key={i} style={{ background: `${data.color}22`, color: data.color, fontWeight: 700, padding: '0 2px', borderRadius: '2px' }}>{part}</mark>
                        : part
                )}
            </span>
        );
    ...
    const handleCategoryClick = (catKey: string | null) => {
        ...
        setSearchQuery('');
        setCurrentPage(1);
        trackEvent('click', { action: 'category_filter_click', category: catKey || 'all', store_name: data.name });
    };
    ...
    return (
        <>
            {activeFontUrl && <link href={activeFontUrl} rel="stylesheet" />}
                ...
                <div className="store-topbar">
                    <div className="store-topbar-inner">
                        ...
                        <div className="topbar-right">
                            <button className={`store-nav-item ${activeView === 'catalogo' ? 'active-view' : ''}`} onClick={() => setActiveView('catalogo')}>Catálogo</button>
                            <button className={`store-nav-item ${activeView === 'about' ? 'active-view' : ''}`} onClick={() => setActiveView('about')}>Nosotros</button>
                            <button className={`store-nav-item ${activeView === 'careers' ? 'active-view' : ''}`} onClick={() => setActiveView('careers')}>Trabaja con nosotros</button>
                        </div>
                    </div>
                </div>
                            ...
                            <div className="store-search-wrapper">
                                <div className="store-search-inner">
                                    ...
                                    <input
                                        ...
                                        onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); setShowSuggestions(false); (e.target as HTMLInputElement).blur(); } }}
                                        autoComplete="off"
                                    />
                                    {searchQuery && (
                                        <button className="store-search-clear" onClick={() => { setSearchQuery(''); setShowSuggestions(false); }} aria-label="Limpiar búsqueda" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>×</button>
                                    )}
                                        ...
                                        <div className="search-suggestions">
                                            {searchProducts(products, searchQuery, { limit: 6, threshold: 10 })
                                                .map(({ product, matchedKeywords }) => (
                                                    <button key={product.id} className="suggestion-item" onClick={() => {
                                                        setSearchQuery(product.name || '');
                                                        setShowSuggestions(false);
                                                        const pCatKey = (product.category || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
                                                        if (activeCategory && pCatKey !== activeCategory) setActiveCategory(null);
                                            ...
                                            <div className="suggestion-footer">Resultados para "{searchQuery}"</div>
                                        </div>
                                    )}
                                </div>
                            </div>
                                            ...
                                            <div key={product.id} className="product-card">
                                                <div className="product-image" style={{ position: 'relative', overflow: 'hidden', cursor: 'pointer' }} onClick={() => setSelectedProduct(product)}>
                                                    {product.images && product.images.length > 0
                                                        ? <img src={product.images[0]} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} loading={index < 4 ? 'eager' : 'lazy'} decoding="async" />
                                                        : product.image
                                                            ? <img src={product.image} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} loading={index < 4 ? 'eager' : 'lazy'} decoding="async" />
                                                            : <div style={{ color: '#ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }}>Sin Imagen</div>}
                                                </div>
                                                <div className="product-details">
                                                    <div className="product-category">{product.category}</div>
                                                    <div className="product-name" style={{ cursor: 'pointer' }} onClick={() => setSelectedProduct(product)}>
                                                    ...
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '16px' }}>
                                                        <a
                                                            ...
                                                            onClick={() => trackEvent('whatsapp_open', { product_name: product.name, store_name: data.name, price: product.price })}
                                                        >
                                                            <span>📱</span> Comprar ahora
                                                        </a>
                                                        <button
                                                            onClick={() => {
                                                                const sanitizedPrice = typeof product.price === 'string' ? product.price.replace(/\./g, '').replace(/,/g, '') : product.price;
                                                                addItem({ id: String(product.id), name: product.name, price: Number(sanitizedPrice), image: product.image, quantity: 1, storeSlug: data.id ? String(data.id) : 'preview' });
                    ...
                    <section className="about-section view-section" style={{ display: 'block' }}>
                        <div className="about-inner">
                            <div>
                                ...
                                {data.about?.mission && (<div><p className="about-block-title">Nuestro propósito / misión</p><p className="about-text">{data.about?.mission}</p></div>)}
                                {data.about?.vision && (<div><p className="about-block-title">Visión</p><p className="about-text">{data.about?.vision}</p></div>)}
                                {data.about?.values && data.about.values.length > 0 && (<div><p className="about-block-title">Valores</p>{renderList(data.about.values, 'dot')}</div>)}
                                {data.about?.diff && data.about.diff.length > 0 && (<div><p className="about-block-title">Qué nos diferencia</p><ul className="about-diff-list">{data.about.diff.map((item, i) => <li key={i}>{item}</li>)}</ul></div>)}
                                {data.about?.team && (<div><p className="about-block-title">Equipo y cultura</p><p className="about-text">{data.about?.team}</p></div>)}
                            ...
                            {data.about?.timeline && data.about.timeline.length > 0 && (
                                <div><p className="about-block-title">Nuestra historia</p><div className="about-timeline">{data.about.timeline.map((item, i) => { const parts = item.split('—'); if (parts.length > 1) { return (<div key={i} className="about-timeline-item"><strong>{parts[0].trim()}</strong><span>{parts.slice(1).join('—').trim()}</span></div>); } return <div key={i} className="about-timeline-item"><span>{item}</span></div>; })}</div></div>
                            )}
                            {data.about?.gallery && data.about.gallery.length > 0 && (
                                <div className="about-gallery"><p className="about-gallery-title">Galería</p><div className="about-gallery-grid">{data.about.gallery.map((img, i) => (<img key={i} src={img} alt={`Foto empresa ${i}`} loading="lazy" onClick={() => setLightboxImage(img)} />))}</div></div>
                            )}
                        </div>
                    </section>
                )}
                    ...
                    <section className="careers-section view-section" style={{ display: 'block' }}>
                        <div className="careers-inner">
                            ...
                            <div className="careers-cta">
                                <a href={`https://wa.me/${cleanPhone(data.whatsapp || '')}?text=${encodeURIComponent('Hola, me interesa trabajar con ustedes.')}`} target="_blank" rel="noopener noreferrer" style={{ backgroundColor: data.color }}><span>💼</span> {data.careers?.ctaText || 'Trabaja con nosotros'}</a>
                            </div>
                        </div>
                    </section>
                )}
            ...
            {selectedProduct && (
                <div className="product-detail-overlay" onClick={() => setSelectedProduct(null)}>
                    <div className="product-detail-modal" onClick={e => e.stopPropagation()}>
                        <button className="product-detail-close" onClick={() => setSelectedProduct(null)} style={{ position: 'absolute', right: '12px', top: '12px', width: '40px', height: '40px', borderRadius: '50%', background: 'white', boxShadow: '0 2px 8px rgba(0,0,0,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '24px', zIndex: 10, border: 'none', cursor: 'pointer', color: '#333' }}>
                            &times;
                        </button>
                                        ...
                                        {selectedProduct.images.length > 1 && (
                                            <div className="thumbnails-grid grid grid-cols-5 gap-2">
                                                {selectedProduct.images.map((img, i) => (
                                                    <button key={i} onClick={() => setSelectedProduct({ ...selectedProduct, activeImage: img } as any)} className={`aspect-square rounded-lg overflow-hidden border-2 transition-all ${((selectedProduct as any).activeImage || selectedProduct.images![0]) === img ? 'border-green-500 scale-95' : 'border-transparent hover:border-gray-300'}`}>
                        ...
                        <div className="product-detail-info-side">
                            ...
                            <div className="detail-actions">
                                <a href={`https://wa.me/${cleanPhone(data.whatsapp)}?text=${encodeURIComponent(`Hola, quiero este producto:\n\n🛍️ *${selectedProduct.name}*\nPrecio: ${formatPrice(selectedProduct.price, data.currency)}\n\nTalla / Color (si aplica): \n\nMi nombre es:\nDirección:\nMétodo de pago:`)}`} target="_blank" rel="noopener noreferrer" className="btn-detail-cart" style={{ backgroundColor: data.color }}>
                                ...
                                <button className="btn-detail-cart" style={{ background: 'transparent', color: data.color, border: `2px solid ${data.color}` }} onClick={() => {
                                    const sanitizedPrice = typeof selectedProduct.price === 'string' ? selectedProduct.price.replace(/\./g, '').replace(/,/g, '') : selectedProduct.price;
                                    addItem({ id: String(selectedProduct.id), name: selectedProduct.name, price: Number(sanitizedPrice), image: (selectedProduct as any).activeImage || (selectedProduct.images && selectedProduct.images[0]) || selectedProduct.image, quantity: 1, storeSlug: data.id ? String(data.id) : 'preview' });
                                    alert('Agregado al carrito 🛒');
                                }}>
                                    Agregar al carrito
                                </button>
                                <button className="btn-detail-close-mobile" onClick={() => setSelectedProduct(null)}>Volver al catálogo</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            {/* LIGHTBOX */}
            {lightboxImage && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setLightboxImage(null)}>
                    <img src={lightboxImage} alt="Imagen galería" style={{ maxWidth: '90vw', maxHeight: '90vh', borderRadius: '12px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)' }} />
                </div>
            )}
        </>
    );
